...
@push('meta-tag')
<title>{!! !empty($pageInfo) ? $pageInfo->meta_title : 'আর্কাইভ - '.$settingsInfo->title !!}</title>
<meta property="og:title" content="{!! !empty($pageInfo) ? $pageInfo->meta_title : 'আর্কাইভ - '.$settingsInfo->title !!}" />
<meta name="twitter:title" content="{!! !empty($pageInfo) ? $pageInfo->meta_title : 'আর্কাইভ - '.$settingsInfo->title !!}" />
		...
		<div class="row">
			...
			<div class="col-sm-12 col-md-12 marginB20 hidden-xs">
				<div class="borderC1B1">
					<h1 class="desktopCategoryTitle"><strong><span class="leftSpan"></span> আর্কাইভ</strong></h1>
				</div>
			</div>
							...
							<div class="panel panel-default borderRadius5 borderAsh1 marginB0">
								<div class="panel-body bgAsh">
									<div class="row">
										<div class="col-md-6">
											<div>
												<input value="{{isset($_GET['headline'])&& !empty($_GET['headline']) ? $_GET['headline'] : ''}}" name="headline" class="form-control borderRadius5 h50" type="text" placeholder="শিরোনাম" autocomplete="off">
											</div>
										</div>
										<div class="col-md-3 paddingL0 paddingR0 xs-paddingL15 xs-paddingR15 xs-marginT10">
											<div class="input-group date" id="datePicker">
												<input name="date" value="{{isset($_GET['date'])&& !empty($_GET['date']) ? $_GET['date'] : date('Y-m-d')}}" placeholder="তারিখ" class="form-control borderRadius5 h50 selectedArchiveDate" type="text" autocomplete="off">
												<span class="input-group-addon borderRadius5 h50"><i class="fa fa-calendar"></i></span>
											</div>
										</div>
										<div class="col-md-2 xs-marginT10 paddingR0">
										...
										<div class="col-md-1 xs-marginT10">
											<button type="button" title="Download Headlines" class="btn btn-success btn-block borderRadius5 width100P title1_8 colorWhite h50 padding3 headlineDownloadButton"><i class="fa fa-download title1_3"></i></button>
										</div>
									</div>
								</div>
							</div>
	...
	<div class="container">
		<div class="row">
			<div class="col-sm-12 col-md-12 paddingLR20 marginB15 paddingT10 marginT10">
				<p class="sectionTitle displayInline border0 margin0 marginB0"><strong><span class="leftSpan"></span> আর্কাইভ</strong></p>
			</div>
					...
					<div class="panel panel-default borderRadius5 borderAsh1 marginB0">
						<div class="panel-body bgAsh">
							<div class="row">
								<div class="col-md-6">
									<div>
										<input value="{{isset($_GET['headline'])&& !empty($_GET['headline']) ? $_GET['headline'] : ''}}" name="headline" class="form-control borderRadius5 h34" type="text" placeholder="শিরোনাম" autocomplete="off">
									</div>
								</div>
								<div class="col-md-3 paddingL0 paddingR0 xs-paddingL15 xs-paddingR15 xs-marginT10">
									<div class="input-group date" id="datePicker">
										<input name="date" value="{{isset($_GET['date'])&& !empty($_GET['date']) ? $_GET['date'] : date('Y-m-d')}}" placeholder="তারিখ" class="form-control borderRadius5 h34 selectedArchiveDate" type="text" autocomplete="off">
										<span class="input-group-addon borderRadius5 h34"><i class="fa fa-calendar"></i></span>
									</div>
								</div>
								<div class="col-xs-6 xs-marginT10 paddingR0">
								...
								<div class="col-xs-6 xs-marginT10">
									<button type="button" title="Download Headlines" class="btn btn-block btn-success borderRadius5 title1_8 colorWhite h34 padding3 headlineDownloadButton"><i class="fa fa-download title1_3"></i></button>
								</div>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	$('.headlineDownloadButton').click(function(){
		var selectedArchiveDate = $('.selectedArchiveDate').val();
		if(selectedArchiveDate != ''){
			window.location.href = '{!! url('archive/print') !!}?date='+selectedArchiveDate;
		}
	})
</script>
@endpush