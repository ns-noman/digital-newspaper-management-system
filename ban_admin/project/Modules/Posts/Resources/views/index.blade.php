...
<section class="vbox">
	<section class="scrollable padder">
		...
		<section class="panel panel-default">
			<header class="panel-heading font-bold">Post | List 
				<a href="{{route('Posts Create')}}" class="btn btn-xs btn-success pull-right"><i class="fa fa-plus"></i> New Post</a>
			...
			<div class="row wrapper">
				<form method="get">
					...
					<div class="col-sm-2" style="margin-top: 10px">
						<select class="form-control" name="custom_xml">
							<option value="">-Select Evergreen-</option>
							<option value="1" {!! isset($_GET['custom_xml']) && ($_GET['custom_xml'] == 1) ? 'selected' : '' !!}>Marked As Evergreen</option>
						</select>
					</div>

					<div class="col-sm-1" style="padding-left: 0px;margin-top: 10px">
						<button type="submit" class="btn btn-default" style="width: 100%">Search</button>
					</div>
				</form>
			</div>

			<form method="post" action="{{route('Posts Bulk Update')}}">
				{{csrf_field()}}
					...
					<table class="table table-striped b-t b-light table-bordered table-hover">
						...
						<tbody class="dragAndDrop">
							...
							<tr class="{{!empty($article->editor_taken) ? 'editorWorkingDiv' : ''}}">
								...
								<td>
									<a href="{!! !empty($article->category_id) && !empty($article->parentCategory) ?  env('WEBSITE').$article->parentCategory->title.'/'.$article->id : '#' !!}" style="color: {!! !empty($article->headline_color) ? $article->headline_color : '#177bbb' !!}" target="_blank">
										@if($article->liveupdate_status == 1)<p><span class="primaryNote"><img src="{{asset('assets/images/liveicon.gif')}}" alt="Live Icon" style="height: 10px;border-radius: 50%;margin-top: -2px;"> সরাসরি</span></p>@endif
								...
								<td class="text-center">
									...
									@php $t_count++; @endphp
									<span class="btn btn-xs btn-primary font-bold" style="width: 30px">T{{Request::is('posts/leadtop') ? $t_count : ''}}</span>
									@endif
								</td>
								...
								<td>
									...
									@endforeach
									@endif
									@if(!empty($article->selected_status) || !empty($article->selected2_status))
									<p class="marginB0 marginT5">
										@if(!empty($article->selected_status))
										<span style="background-color: #177bbb;color: white;padding: 3px 10px 1px 10px;border-radius: 3px">নির্বাচিত</span>
										@endif
										@if(!empty($article->selected2_status))
										<span style="background-color: #fcbf1a;color: white;padding: 3px 10px 1px 10px;border-radius: 3px">নির্বাচিত ২</span>
										@endif
									</p>
									@endif
								</td>
								...
								<td>{!! App\Http\Controllers\CommonController::articleHit($article->created_at, $article->id, 0) !!}</td>

								<td class="text-center">
									@if($article->id > env('Old_NewsId'))
									<a href="{{route('Posts Edit', $article->id)}}" class="btn btn-primary btn-xs" title="Edit" ><i class="fa fa-edit"></i></a> 
									@endif
									<a data-toggle="modal" data-design="1" data-newsid="{!! $article->id !!}" data-thumbnail="{{env('New_AssetLink').date('/Y/m/d/', strtotime($article->created_at)).$article->thumbnail}}" data-target="#photoCardModal" class="btn btn-success btn-xs photoCardModal" title="Photo Card" ><i class="fa fa-image"></i></a>
								</td>
								<td class="text-center">
									@if($article->status)
									...
									@endif
									@if($article->paidnews_status == 1)
									<br>
									<button type="button" class="btn btn-warning btn-xs"><i class="fa fa-money"></i></button>
									@endif
								</td>
							</tr>
							@endforeach
					...
					<div class="row">
						<div class="col-sm-4">
							<select class="input-sm form-control input-s-sm inline v-middle bulkAction" name="bulkStatus" required="">
								...
								<option value="no_placement">Remove Lead/Top</option>
								<option value="0" class="optionGroup" disabled="">Selected</option>
								<option value="Selected">Mark Selected</option>
								<option value="remove_Selected">Remove Selected</option>
								<option value="Selected2">Mark Selected 2</option>
								<option value="remove_Selected2">Remove Selected 2</option>
								<option value="0" class="optionGroup" disabled="">Other Options</option>
								@if(Request::is('posts/leadtop'))
								<option value="swapOrderLeadTop">Swap Order</option>
								@else
								<option value="swapOrder">Swap Order</option>
								@endif
								<option value="releaseArticle">Release Article</option>
								<option value="0" class="optionGroup" disabled="">Paid Status</option>
								<option value="markPaid">Mark Paid</option>
								<option value="removePaid">Remove Paid</option>
...
<!-- photocard modal -->
@include('tools::photo-cards.photocards.cardLayout')
...
<script type="text/javascript">
	$('.bulkAction').change(function(){
		...
		if(bulkAction == 'swapOrder' || bulkAction == 'swapOrderLeadTop'){
