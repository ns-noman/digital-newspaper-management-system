...
class EmployeeInitialsTypeController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $data['lists'] = EmployeeInitialsType::whereIn('status', [1,2])
        ...
        return view('employeeinitials::types', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect()->route('EmployeeInitials Type')->withErrors($validator)->withInput()->with('error_message','Form validation failed!.');
        }

        try{
            DB::beginTransaction();

            $dataInfo = new EmployeeInitialsType;
            ...
            return redirect()->route('EmployeeInitials Type')->with('success_message', 'EmployeeInitial Type has been created successfully!.');

        }catch(Exception $e){
          DB::rollback();
          return redirect()->route('EmployeeInitials Type')->with('error_message', 'Failed to create EmployeeInitial Type!.');
      }
  }
    ...
    public function update(Request $request)
    {
        ...
        try{
            ...
            $id = $request->id;
            $dataInfo = EmployeeInitialsType::find($id);
            $dataInfo->title = $request->title;
            $dataInfo->status = !empty($request->status) ? $request->status : -1;
            ...
            return redirect()->route('EmployeeInitials Type')->with('success_message', 'EmployeeInitial Type has been updated successfully!.');

        }catch(Exception $e){
          DB::rollback();
          return redirect()->route('EmployeeInitials Type')->with('error_message', 'Failed to update EmployeeInitial Type!.');
      }
  }
    ...
    public function orderID(){
      $orderIDInfo = EmployeeInitialsType::orderBy('order_id', 'desc')->first();
