...
namespace Modules\MarketingPersons\Http\Controllers;
...
class MarketingPersonsController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $data['lists'] = MarketingPersons::whereIn('status', [1,2])
        ...
        return view('marketingpersons::index', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect()->route('MarketingPersons')->withErrors($validator)->withInput()->with('error_message','Form validation failed!.');
        }

        try{
            DB::beginTransaction();

            $dataInfo = new MarketingPersons;
            ...
            return redirect()->route('MarketingPersons')->with('success_message', 'MarketingPerson has been created successfully!.');

        }catch(Exception $e){
          DB::rollback();
          return redirect()->route('MarketingPersons')->with('error_message', 'Failed to create MarketingPerson!.');
      }
  }
    ...
    public function update(Request $request)
    {
        ...
        try{
            ...
            $id = $request->id;
            $dataInfo = MarketingPersons::find($id);
            $dataInfo->title = $request->title;
            $dataInfo->contact = $request->contact;
            $dataInfo->email = $request->email;
            $dataInfo->designation = $request->designation;
            ...
            return redirect()->route('MarketingPersons')->with('success_message', 'MarketingPerson has been updated successfully!.');

        }catch(Exception $e){
          DB::rollback();
          return redirect()->route('MarketingPersons')->with('error_message', 'Failed to update MarketingPerson!.');
      }
  }
    ...
    public function orderID(){
        $orderIDInfo = MarketingPersons::orderBy('order_id', 'desc')->first();
