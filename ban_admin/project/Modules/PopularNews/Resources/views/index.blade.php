...
<section class="vbox">
	<section class="scrollable padder">
		...
		<section class="panel panel-default">
			<header class="panel-heading font-bold">Popular News
				<a href="{!! route('PopularNews Pull GA Data') !!}" class="btn btn-danger btn-xs pull-right" style="margin-left: 10px"> <i class="fa fa-paper-plane"></i> Pull GA Data</a>
				<a href="javascript::void(0)" data-toggle="modal" data-target="#createModal" class="btn btn-success btn-xs pull-right"> <i class="fa fa-plus-circle"></i> Manual Entry</a>
			...
			<form method="post" action="{{route('PopularNews Bulk Update')}}">
				{{csrf_field()}}
					...
					<table class="table table-striped b-t b-light table-bordered table-hover">
						...
						<tbody class="dragAndDrop">
							...
							<tr>
								...
								<td><a style="color: blue" href="{!! $list->link !!}" target="_blank">{!! $list->title !!}</a></td>
								<td>{!! $list->total_hit !!}</td>
								<td>{{$list->createdBy->name}}<br>{{$list->created_at}}</td>
								<td>{{$list->status == 1 ? 'Active' : 'Inactive'}}</td>
								<td>
									<button type="button" class="btn btn-primary btn-xs editModal" data-toggle="modal" data-target="#editModal" data-id="{{$list->id}}" data-title="{{$list->title}}" data-link="{{$list->link}}" data-hit="{{$list->total_hit}}" data-status="{{$list->status}}"><i class="fa fa-edit" title="Edit"></i></button>
									<a href="{{route('PopularNews Delete', $list->id)}}" class="btn btn-danger btn-xs delete" onclick="return confirm('Are you sure you want to  delete?');"><i class="fa fa-trash"></i></a>
								</td>
							</tr>
							@endforeach
							@else
		...
		<div class="content">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">Add Popular News</h4>
				</div>
				<div class="modal-body">
					<form role="form" class="form-horizontal" method="post" action="{{route('PopularNews Store')}} " enctype="multipart/form-data">
						{{csrf_field()}}
						...
						<div class="form-group">
							<div class="col-sm-3 text-right">
								<label class="control-label"><b>Total Hit*</b></label>
							</div>
							<div class="col-sm-8">
								<input type="number" class="form-control" name="total_hit" required="" />
							</div>
						</div>
	...
	<div class="modal-dialog">
		<div class="content">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title">Edit Popular News</h4>
				</div>
				<div class="modal-body">
					<form role="form" class="form-horizontal" method="post" action="
					{{route('PopularNews Update')}}" enctype="multipart/form-data">
					{{csrf_field()}}
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="number" class="form-control" name="total_hit" id="total_hit" required="" />
						</div>
					</div>
...
<script type="text/javascript">
	$(".editModal").click(function(){
		...
		var total_hit = $(this).data('hit');
		var status = $(this).data('status');

		$('.modal-body #id').val(id);
		$('.modal-body #title').val(title);
		$('.modal-body #link').val(link);
		$('.modal-body #total_hit').val(total_hit);
