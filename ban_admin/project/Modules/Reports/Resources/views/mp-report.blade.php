@extends('layouts.app')
@section('title', 'Marketing Person Report')
		...
		<div class="row">
			<div class="col-md-12">
				<section class="panel panel-default">
					<header class="panel-heading font-bold">Report: Marketing Person Report</header>
					...
					<div class="panel-body">
						<div class="table-responsive">
							<table class="table table-bordered table-striped marginB0">
								<thead>
									<tr>
										<th colspan="3">Duration: {!! isset($_GET['dateFrom']) ? date('d M Y', strtotime($_GET['dateFrom'])) : '' !!} - {!! isset($_GET['dateTo']) ? date('d M Y', strtotime($_GET['dateTo'])) : '' !!}, Marketing Person: {!! isset($_GET['marketing_person_id']) && !empty($_GET['marketing_person_id']) && !empty($marketingPersonInfo) ? $marketingPersonInfo->title : 'ALL' !!}, Marketing Company: {!! isset($_GET['marketing_company_id']) && !empty($_GET['marketing_company_id']) && !empty($marketingCompanyInfo) ? $marketingCompanyInfo->title : 'ALL' !!}</th>
									</tr>
									<tr>
										<th>SL</th>
								...
								<tbody>
									@if(!empty($mpReports) && (count($mpReports)>0))
									@php $totalNews = 0; @endphp
									@foreach($mpReports as $key => $mpReport)
									@php $totalNews = $totalNews+$mpReport->total_news; @endphp
									<tr>
										<td>{!! $key+1 !!}</td>
										<td>{!! $mpReport->title !!}</td>
										<td>
											<a class="btn btn-primary btn-xs hidden-print" href="{!! str_replace('marketing_person_id=', 'marketing_person_id='.$mpReport->id ,str_replace('mp-report', 'pr-report/news', Request::fullUrl())) !!}">{!! $mpReport->total_news !!}</a>
											<span class="visible-print">{!! $mpReport->total_news !!}</span>
										</td>
									</tr>
									@endforeach
									<tr>
