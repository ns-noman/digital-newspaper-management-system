@extends('layouts.app')
@section('title', 'Marketing Company Report')
		...
		<div class="row">
			<div class="col-md-12">
				<section class="panel panel-default">
					<header class="panel-heading font-bold">Report: Marketing Company Report</header>
					...
					<div class="panel-body">
						<div class="table-responsive">
							<table class="table table-bordered table-striped marginB0">
								...
								<tbody>
									@if(!empty($mcReports) && (count($mcReports)>0))
									@php $totalNews = 0; @endphp
									@foreach($mcReports as $key => $mcReport)
									@php $totalNews = $totalNews+$mcReport->total_news; @endphp
									<tr>
										<td>{!! $key+1 !!}</td>
										<td>{!! $mcReport->title !!}</td>
										<td>
											<a class="btn btn-primary btn-xs hidden-print" href="{!! str_replace('marketing_company_id=', 'marketing_company_id='.$mcReport->id ,str_replace('mc-report', 'pr-report/news', Request::fullUrl())) !!}">{!! $mcReport->total_news !!}</a>
											<span class="visible-print">{!! $mcReport->total_news !!}</span>
										</td>
									</tr>
									@endforeach
									<tr>
