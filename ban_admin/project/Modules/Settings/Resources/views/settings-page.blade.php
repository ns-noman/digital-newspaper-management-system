...
<section class="vbox">
	<section class="scrollable padder">
		...
		<section class="panel panel-default">
			<header class="panel-heading font-bold">Page Settings
				<a href="javascript::void(0)" data-toggle="modal" data-target="#createModal" class="btn btn-success btn-xs pull-right"> <i class="fa fa-plus-circle"></i> New Page</a>
			...
			<div class="row wrapper">
				<form method="get">
					...
					<div class="col-sm-2 m-b-xs paddingL0">
						<input name="slug" type="text" class="input-sm form-control" value="{{!empty($_GET['slug']) ? $_GET['slug'] : ''}}" placeholder="Slug" autocomplete="off" />
					</div>
			...
			<form method="post" action="{{route('Settings Page Bulk Update')}}">
				{{csrf_field()}}
					...
					<table class="table table-striped b-t b-light table-bordered table-hover">
						...
						<tbody class="dragAndDrop">
							...
							<tr>
								...
								<td>
									<a style="color: #177bbb" href="{!! env('WEBSITE').$list->slug !!}" target="_blank">{!! $list->title !!}</a>
									<p><u>Slug:</u> {!! $list->slug !!}</p>
								</td>
								<td>{!! $list->meta_title !!}</td>
								<td>
									@if(!empty($list->meta_image))
									<img src="{{env('UploadsLink').'uploads/pages/'.$list->meta_image}}" width="50px" class="img-responsive" />
									@endif
								</td>
								...
								<td>
									<button type="button" class="btn btn-primary btn-xs openeditModal" data-toggle="modal" data-target="#editModal" data-id="{{$list->id}}" data-title ="{{$list->title }}" data-slug ="{{$list->slug }}" data-meta-title="{{$list->meta_title}}" data-meta-image="{{$list->meta_image}}" data-meta-descriptions="{{$list->meta_descriptions}}" data-meta-keywords="{{$list->meta_keywords}}" data-header-code="{{$list->header_code}}" data-status="{{$list->status}}"><i class="fa fa-edit" title="Edit"></i></button>
									<a href="{{route('Settings Page Delete', $list->id)}}" class="btn btn-danger btn-xs delete" onclick="return confirm('Are you sure you want to  delete?');"><i class="fa fa-trash"></i></a>
								</td>
		...
		<div class="content">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form role="form" class="form-horizontal" method="post" action="{{route('Settings Page Store')}}" enctype="multipart/form-data">
						{{csrf_field()}}

						<div class="form-group">
							<label class="col-sm-3 control-label"><b>Title *</b></label>
							<div class="col-sm-8">
								<input type="text" name="title" class="form-control" autofocus required="" placeholder="Ex: About Us" />
								@if($errors->has('title'))
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" name="slug" class="form-control" autofocus required="" placeholder="Ex: about-us" />
								@if($errors->has('slug'))
						...
						<div class="form-group">
							<div class="col-sm-3 text-right">
								<label class="control-label"><b>Meta Photo (1200x675)</b></label>
							</div>
							<div class="col-sm-8">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label"><b>Header Code</b></label>
							<div class="col-sm-8">
								<textarea type="text" name="header_code" class="form-control" rows="4"></textarea>
	...
	<div class="modal-dialog modal-lg">
		<div class="content">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form role="form" class="form-horizontal" method="post" action="{{route('Settings Page Update')}} " enctype="multipart/form-data">
						{{csrf_field()}}
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" name="slug" id="slug" class="form-control" autofocus required="" placeholder="Ex: about-us" />
								@if($errors->has('slug'))
								<span class="help-block">{{ $errors->first('slug') }}</span>
								@endif
							</div>
						...
						<div class="form-group">
							<label class="col-sm-3 control-label"><b>Header Code</b></label>
							<div class="col-sm-8">
								<textarea type="text" name="header_code" id="header_code" class="form-control" rows="4"></textarea>
								@if($errors->has('header_code'))
								<span class="help-block">{{ $errors->first('header_code') }}</span>
								@endif
							</div>
...
<script type="text/javascript">
	$(".openeditModal").click(function(){
		...
		var header_code = $(this).data('header-code');
		...
		$('.modal-body #header_code').val(header_code);
		$('.modal-body #status').val(status);

		if(meta_image != ''){
			var metaimage = '{{env('UploadsLink')}}/uploads/pages'+'/'+meta_image;
			$(".modal-body #meta_image").attr('src', metaimage);
		}else{
			var metaimage = '{{env('UploadsLink')}}/uploads/settings/thumbnail.jpg';
			$(".modal-body #meta_image").attr('src', metaimage);
		}
