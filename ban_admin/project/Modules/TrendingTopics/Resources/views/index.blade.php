...
<section class="vbox">
	<section class="scrollable padder">
		...
		<section class="panel panel-default">
			<header class="panel-heading font-bold">Trending Topics
				<a href="javascript::void(0)" data-toggle="modal" data-target="#createModal" class="btn btn-success btn-xs pull-right"> <i class="fa fa-plus-circle"></i> New Trending Topics</a>
			...
			<form method="post" action="{{route('TrendingTopics Bulk Update')}}">
				{{csrf_field()}}
					...
					<table class="table table-striped b-t b-light table-bordered table-hover">
						...
						<tbody class="dragAndDrop">
							...
							<tr>
								...
								<td>
									<a style="color: #177bbb" href="{!! env('WEBSITE').'news-issue/'.$list->slug !!}" target="_blank">{!! $list->title !!}</a>
								...
								<td>
									<button type="button" class="btn btn-primary btn-xs editModal" data-toggle="modal" data-target="#editModal" data-id="{{$list->id}}" data-title="{{$list->title}}" data-slug="{{$list->slug}}" data-tag="{{$list->tag}}" data-status="{{$list->status}}"><i class="fa fa-edit" title="Edit"></i></button>
									<a href="{{route('TrendingTopics Delete', $list->id)}}" class="btn btn-danger btn-xs delete" onclick="return confirm('Are you sure you want to  delete?');"><i class="fa fa-trash"></i></a>
								</td>
							</tr>
							@endforeach
							@else
		...
		<div class="content">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">Add New Trending Topic</h4>
				</div>
				<div class="modal-body">
					<form role="form" class="form-horizontal" method="post" action="{{route('TrendingTopics Store')}} " enctype="multipart/form-data">
						{{csrf_field()}}
						...
						<div class="form-group">
							<div class="col-sm-3 text-right">
								<label class="control-label"><b>Url Slug*</b></label>
							</div>
							<div class="col-sm-8">
	...
	<div class="modal-dialog">
		<div class="content">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title">Edit Trending Topic</h4>
				</div>
				<div class="modal-body">
					<form role="form" class="form-horizontal" method="post" action="
					{{route('TrendingTopics Update')}}" enctype="multipart/form-data">
					{{csrf_field()}}
