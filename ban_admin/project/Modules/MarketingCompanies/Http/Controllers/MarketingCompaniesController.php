...
namespace Modules\MarketingCompanies\Http\Controllers;
...
class MarketingCompaniesController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $data['lists'] = MarketingCompanies::whereIn('status', [1,2])
        ...
        ->when(isset($_GET['contact']) && !empty($_GET['contact']), function ($query) {
            $query->where('contact', 'like', '%'.$_GET['contact'].'%');
        })
        ...
        return view('marketingcompanies::index', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect()->route('MarketingCompanies')->withErrors($validator)->withInput()->with('error_message','Form validation failed!.');
        }

        try{
            DB::beginTransaction();

            $dataInfo = new MarketingCompanies;
            ...
            return redirect()->route('MarketingCompanies')->with('success_message', 'MarketingCompany has been created successfully!.');

        }catch(Exception $e){
          DB::rollback();
          return redirect()->route('MarketingCompanies')->with('error_message', 'Failed to create MarketingCompany!.');
      }
  }
    ...
    public function update(Request $request)
    {
        ...
        try{
            ...
            $id = $request->id;
            $dataInfo = MarketingCompanies::find($id);
            $dataInfo->title = $request->title;
            $dataInfo->contact = $request->contact;
            $dataInfo->email = $request->email;
            $dataInfo->website = $request->website;
            ...
            return redirect()->route('MarketingCompanies')->with('success_message', 'MarketingCompany has been updated successfully!.');

        }catch(Exception $e){
          DB::rollback();
          return redirect()->route('MarketingCompanies')->with('error_message', 'Failed to update MarketingCompany!.');
      }
  }
    ...
    public function orderID(){
        $orderIDInfo = MarketingCompanies::orderBy('order_id', 'desc')->first();
