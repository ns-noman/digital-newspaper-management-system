...
<section class="vbox">
	<section class="scrollable padder">
		...
		<section class="panel panel-default">
			<header class="panel-heading font-bold">Polls
				<a href="javascript::void(0)" data-toggle="modal" data-target="#createModal" class="btn btn-success btn-xs pull-right"> <i class="fa fa-plus-circle"></i> New Poll</a>
			...
			<div class="row wrapper">
				<form method="get">
					...
					<div class="col-sm-2 m-b-xs">
						<input name="question" type="text" class="input-sm form-control" value="{{!empty($_GET['question']) ? $_GET['question'] : ''}}" placeholder="Question" autocomplete="off" />
					</div>
			...
			<form method="post" action="{{url('polls/bulkupdate')}}">
				{{csrf_field()}}
					...
					<table class="table table-striped b-t b-light table-bordered table-hover">
						<thead>
							<tr>
								...
								<th class="text-center">No Opinion</th>
								<th width="120">Poll Date</th>
								<th>Create</th>
								<th>Update</th>
								<th>Status</th>
						...
						<tbody class="dragAndDrop">
							...
							<tr>
								...
								<td width="80">
									@if(!empty($list->image))
									<img src="{!! env('UploadsLink').'uploads/polls/'.$list->image !!}" style="width: 80px">
									@endif
								</td>
								<td class="text-center">{!! $list->total_vote !!}</td>
								<td class="text-center">{!! $list->yes_vote !!}</td>
								<td class="text-center">{!! $list->no_vote !!}</td>
								<td class="text-center">{!! $list->no_opinion !!}</td>
								<td>{!! date('d M, Y', strtotime($list->poll_date)) !!}</td>
