...
namespace Modules\PhotoCarts\Http\Controllers;
...
class PhotoCartsController extends Controller
{
    /**
    ...
    public function generatePhotoCart($newsId){
      ...
      if(!empty($newsInfo) && !empty($imageUrl)){
        ...
        return view('photocarts::photocart1', $data);
      }
    }
    ...
    //   $data['lists'] = PhotoCarts::whereIn('status', [1,2])
  ...
  //       'default_status' => 'nullable|numeric|between:-1,2',
  //     ]);

  //     if ($validator->fails()) {
  //       return redirect()->route('PhotoCarts')->withErrors($validator)->withInput()->with('error_message','Form validation failed!.');
  //     }

  //     try{
  //       DB::beginTransaction();

  //       $dataInfo = new PhotoCarts;
  //       $dataInfo->title = $request->title;
  //       $dataInfo->order_id = $this->orderID();
  //       $dataInfo->default_status = !empty($request->default_status) ? $request->default_status : Null;
  ...
  //          $fileName = time().'-'.uniqid().".".$frame->getClientOriginalExtension();
  //          $savingPath = env('NewAssetFolderPath').'images/photoframes/';
  //          Storage::disk(env('DISK'))->putFileAs($savingPath, $frame, $fileName);
